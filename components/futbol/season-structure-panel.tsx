...
import { listRounds, replaceRounds } from "@/lib/api/admin-soccer";
...
export function SeasonStructurePanel({ seasonId }: { seasonId: string }) {
  ...
  const [phaseName, setPhaseName] = useState("Fase Regular");
  const [generateCount, setGenerateCount] = useState(17);
  ...
  function generateJornadas() {
    const generated = Array.from({ length: generateCount }, (_, i) => ({
      name: `Jornada ${i + 1}`,
      position: i + 1,
      phase: phaseName || "Fase Regular",
    }));
    setRows(generated);
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-dx-muted">
            Fases y jornadas · no es obligatorio precrear toda la estructura
          </p>
        </div>
        <div className="flex flex-wrap items-end gap-2">
          ...
          <Button variant="secondary" onClick={generateJornadas}>
            Generar jornadas
          </Button>
          <Button
            onClick={() =>
      ...
      <Note>
        Ejemplo Liga MX: Fase Regular → Jornada 1…17; Liguilla → Cuartos,
        Semifinales, Final.
      </Note>
