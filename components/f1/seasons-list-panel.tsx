...
export function SeasonsListPanel({ competitionId }: { competitionId: string }) {
  ...
  const seasonsQuery = useQuery({
    queryKey: ["admin", "f1", "seasons", competitionId],
    queryFn: () => listAdminF1Seasons(competitionId),
  });
  ...
  if (seasonsQuery.isLoading || competitionQuery.isLoading) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-dx-muted">
            Vigentes y próximas primero · {competitionName}
          </p>
        </div>
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {active.length === 0 ? (
          <p className="text-sm text-dx-muted">No hay temporadas vigentes/próximas.</p>
        ) : null}
      </div>
