...
import type { NflCountry, NflLeague } from "@/lib/api/nfl-types";
...
export function StructureMasterPanel() {
  ...
  const getCountryValues = useCallback((row: NflCountry) => {
    return {
      name: row.name ?? "",
      code: row.code ?? "",
    };
  ...
  const getLeagueValues = useCallback((row: NflLeague) => {
    return {
      name: row.name ?? "",
  ...
  if (countriesQuery.isLoading && leaguesQuery.isLoading) {
